...
export async function generateAIMoodMatch(mood: string, language: Language): Promise<{
  ...
  try {
    ...
    // For each recommended title, try to search TMDb to get real poster, ID, rating, overview
    const matchedMoviesPromises = recommendedList.map(async (rec) => {
      ...
      if (matched) {
        ...
        // Fallback placeholder card if TMDb didn't return a match
        return {
...
export async function sendAIChatMessage(
  ...
  try {
    ...
    // Enrich movie cards with real TMDb data
    const moviePromises = rawCards.map(async (card: any) => {
      ...
      if (matched) {
        return {
          ...matched,
          aiMatchReason: card.matchReason || card.summary,
        ...
        return {
          ...
          title: card.title,
          overview: card.summary || card.matchReason,
          poster_path: null,
          backdrop_path: null,
          release_date: card.year || '2024',
          vote_average: card.rating || 8.5,
          vote_count: 1000,
